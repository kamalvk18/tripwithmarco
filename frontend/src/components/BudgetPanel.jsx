...
export function BudgetPanel({ breakdown, userBudget, currency }) {
    ...
    return (
      <div className="rounded-xl border border-slate-200 bg-white shadow-sm p-5">
        ...
        <div className="flex justify-between items-center">
          ...
          <span className="text-base font-bold text-slate-800">{formatMoney(userBudget, currency)}</span>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-white shadow-sm p-5">
      <h3 className="text-sm font-semibold text-slate-700 mb-4">💰 Budget Estimate</h3>
          ...
          return (
            <div key={key}>
              <div className="flex justify-between text-xs mb-1.5">
                <span className="text-slate-500">{icon} {key.charAt(0).toUpperCase() + key.slice(1)}</span>
                <span className="text-slate-700 font-medium">{formatMoney(val, currency)}</span>
              </div>
      ...
      <div className="mt-4 pt-3 border-t border-slate-100 flex justify-between items-center">
        ...
        <span className="text-base font-bold text-slate-800">{formatMoney(total, currency)}</span>
      </div>

      {userBudget > 0 && (
        <div className="mt-2 flex justify-between items-center">
          <span className="text-sm text-slate-500">Your budget</span>
          <span className="text-sm text-slate-600">{formatMoney(userBudget, currency)}</span>
        </div>
      )}
